...
class Squirrel:
    ...
    def update(self, userInput):
        ...
        if (userInput[pygame.K_UP] or userInput[pygame.K_SPACE]) and not self.squirrel_jump and self.Y_POS == 350:
            self.squirrel_duck = False
    ...
    def jump(self):
        ...
        if self.squirrel_jump:
            self.squirrel_rect.y -= self.jump_vel * 4
            self.jump_vel -= 0.6
    ...
    def jump2(self):
        self.image = self.jump_img[self.step_indexj]
        if self.squirrel_jump2:
            self.squirrel_rect.y -= self.jump_vel * 3
            self.jump_vel -= 2.6
        if self.jump_vel < -self.JUMP_VEL:
            self.squirrel_jump2 = False
            self.jump_vel = self.JUMP_VEL
